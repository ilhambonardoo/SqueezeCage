...
import BoxModel from "../utils/Box";
...
const Model3DSection = () => {
  ...
  const [isShowKerangka, setIsSetShowKerangka] = useState(false);
  const [isShowBox, setIsSetShowBox] = useState(false);
      ...
      <section className="w-full bg-white lg:py-24 pt-5 pb-10 px-4 md:px-8">
        <div className="flex flex-col gap-5">
          <div className="flex flex-col lg:flex-row gap-5">
            <div
              className="relative max-w-7xl group overflow-hidden rounded-2xl md:rounded-3xl flex cursor-pointer shadow-2xl lg:w-1/2"
              onClick={() => {
                setIsSetShowKerangka(true);
              }}
            >
              <video
                className="w-full rounded-2xl cursor-pointer md:rounded-3xl shadow-2xl h-75 md:h-auto lg:h-96 object-cover aspect-video md:aspect-21/9"
                autoPlay
                muted
                loop
                playsInline
              ...
              <div className="absolute inset-0 bg-black/40 flex flex-col items-center justify-center gap-2 md:gap-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                ...
                >
                  <BoxIcon className="w-8 h-8 md:w-12 md:h-12 text-white" />
                </motion.div>
            ...
            <div
              className="relative group overflow-hidden rounded-2xl md:rounded-3xl cursor-pointer shadow-2xl lg:w-1/2"
              onClick={() => {
                setIsSetShowBox(true);
              }}
            >
              <Image
                src={"/3D/box.webp"}
                ...
                width={300}
                height={368}
                className="w-full h-75 md:h-auto lg:h-96 object-cover"
              />
