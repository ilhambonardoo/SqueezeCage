...
const baseTernakSchema = z.object({
...
export class TernakValidation {
  static readonly CREATE = baseTernakSchema.extend({
    imageKey: z.string().optional().or(z.literal("")),
  });
  static readonly UPDATE = baseTernakSchema.partial().extend({
    imageKey: z
      .union([z.string(), z.null(), z.literal("").nullable()])
      .optional(),
  });
}
